...
fn bench_is_rate_limited(b: &mut Bencher, test_scenario: &TestScenario, storage: Box<dyn Storage>) {
    ...
    b.iter(|| {
        ...
        black_box(
            rate_limiter
                .is_rate_limited(&params.namespace, &params.values, params.delta)
                .unwrap(),
        )
    })
}
...
fn bench_update_counters(b: &mut Bencher, test_scenario: &TestScenario, storage: Box<dyn Storage>) {
    ...
    b.iter(|| {
        ...
        black_box(
            rate_limiter
                .update_counters(&params.namespace, &params.values, params.delta)
                .unwrap(),
        )
    })
}
...
fn bench_check_rate_limited_and_update(
    ...
    b.iter(|| {
        ...
        black_box(
            rate_limiter
                .check_rate_limited_and_update(&params.namespace, &params.values, params.delta)
                .unwrap(),
        )
    })
}
